/*******************************************************************************
* File Name: LCD_Screen_BarGraph.c
* Version 2.20
*
* Description:
...
uint8 const CYCODE LCD_Screen_customFonts[] = \
{
    /* Character LCD_Screen_CUSTOM_0   */
    0x00u,    0x00u,    0x00u,    0x00u,    0x00u,    0x00u,    0x00u,    0x00u, \
    /* Character LCD_Screen_CUSTOM_1   */
    0x10u,    0x10u,    0x10u,    0x10u,    0x10u,    0x10u,    0x10u,    0x10u, \
    /* Character LCD_Screen_CUSTOM_2   */
    0x18u,    0x18u,    0x18u,    0x18u,    0x18u,    0x18u,    0x18u,    0x18u, \
    /* Character LCD_Screen_CUSTOM_3   */
    0x1Cu,    0x1Cu,    0x1Cu,    0x1Cu,    0x1Cu,    0x1Cu,    0x1Cu,    0x1Cu, \
    /* Character LCD_Screen_CUSTOM_4   */
    0x1Eu,    0x1Eu,    0x1Eu,    0x1Eu,    0x1Eu,    0x1Eu,    0x1Eu,    0x1Eu, \
    /* Character LCD_Screen_CUSTOM_5   */
    0x1Fu,    0x1Fu,    0x1Fu,    0x1Fu,    0x1Fu,    0x1Fu,    0x1Fu,    0x1Fu, \
    /* Character LCD_Screen_CUSTOM_6   */
    0x00u,    0x00u,    0x00u,    0x00u,    0x00u,    0x00u,    0x00u,    0x00u, \
    /* Character LCD_Screen_CUSTOM_7   */
    0x00u,    0x00u,    0x00u,    0x00u,    0x00u,    0x00u,    0x00u,    0x00u \
};


/*******************************************************************************
* Function Name: LCD_Screen_LoadCustomFonts
********************************************************************************
*
* Summary:
...
void LCD_Screen_LoadCustomFonts(uint8 const customData[]) 
{
    ...
    LCD_Screen_WriteControl(LCD_Screen_CGRAM_0);

    /* Load in the 64 bytes of CustomChar Data */
    for(indexU8 = 0u; indexU8 < LCD_Screen_CUSTOM_CHAR_SET_LEN; indexU8++)
    {
        /* Delay between each write */
        LCD_Screen_WriteData(customData[indexU8]);
    }

    LCD_Screen_IsReady();
    LCD_Screen_WriteControl(LCD_Screen_DDRAM_0);
}


#if (LCD_Screen_CUSTOM_CHAR_SET == LCD_Screen_HORIZONTAL_BG)

    /*******************************************************************************
    * Function Name: LCD_Screen_DrawHorizontalBG
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        ...
        fullChars = value / LCD_Screen_CHARACTER_WIDTH;

        /* Number of remaining pixels to draw */
        remainingPixels = value % LCD_Screen_CHARACTER_WIDTH;
        ...
        LCD_Screen_Position(row, column);

        /* Write full characters */
        for(count8 = 0u; count8 < fullChars; count8++)
        {
            LCD_Screen_WriteData(LCD_Screen_CUSTOM_5);
        }

        if(fullChars < maxCharacters)
        {
            /* Write remaining pixels */
            LCD_Screen_WriteData(remainingPixels);
            ...
            if(fullChars < (maxCharacters - 1u))
            {
                ...
                {
                    LCD_Screen_WriteData(LCD_Screen_CUSTOM_0);
                }
            }
        }
    }

#endif /* LCD_Screen_CUSTOM_CHAR_SET == LCD_Screen_HORIZONTAL_BG */


#if (LCD_Screen_CUSTOM_CHAR_SET == LCD_Screen_VERTICAL_BG)

    /*******************************************************************************
    *  Function Name: LCD_Screen_DrawVerticalBG
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        ...
        /* Number of full characters to draw */
        fullChars = value / LCD_Screen_CHARACTER_HEIGHT;

        /* Number of remaining pixels to draw */
        remainingPixels = value % LCD_Screen_CHARACTER_HEIGHT;
        ...
        {
            LCD_Screen_WriteData(LCD_Screen_CUSTOM_7);
            ...
            {
                LCD_Screen_Position(row - count8, column);
            }
            else
            {
                break;
        ...
        if(((((int8) row) - ((int8) count8)) >= 0) && (fullChars < maxCharacters))
        {
            ...
            {
                LCD_Screen_WriteData((uint8) ' ');
            }
            else
            {
                LCD_Screen_WriteData(remainingPixels - 1u);
            }
            ...
            if(currentRow >= 0)
            {
                ...
                for(count8 = 0u; count8 < (maxCharacters - fullChars - 1u); count8++)
                {
                    LCD_Screen_Position((uint8)currentRow, column);
                    LCD_Screen_WriteData((uint8) ' ');
                    currentRow --;
                }
            }
